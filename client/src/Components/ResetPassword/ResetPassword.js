...
const ResetPassword = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (userId && userPassword) {
      axios
        .patch(`http://localhost:5000/api/v1/users/forgotPassword/${userId}`, {
          password: userPassword,
        })
        .then((res) => alert("password update successfully"))
        .catch((err) =>
    ...
    >
      <Container>
        <Form onSubmit={handleSubmit}>
          ...
          <Link to={"/"}>
          <Button variant="outline-dark" className="ms-2">Home</Button>
        </Link>
        </Form>
